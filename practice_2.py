...
class ImportantSets():
            ...
            for rule in self.__grammar[symbol]:
                ...
                if symbol == rule[0]:
                    if "''" in set_first:
                        ...
                        continue
                aux = self.__get_set_of_the_first(symbol, rule)
    ...
    def __get_set_of_the_first(self, key, rule):
        epsilon = False
        set_first = set()
        aux_set = set()
        aux = rule.split(' ', 1)
        ...
        if key == symbol:
            print('key:{}:symbol:{}:'.format(key, symbol))
            return set_first
        ...
        if len(aux) == 2 and epsilon:
            ...
            aux_set = self.__get_set_of_the_first(key, leftover)
            set_first = set_first | aux_set
    ...
    def __call_following_set_non_terminal(self, symbol):
        if symbol not in self.__dict_of_the_following:
            self.__dict_of_the_following[symbol] = set()
            set_following = set()
            aux_set = set()
            if symbol == 'S'and self.is_empty(self.__dict_of_the_following[
                                              symbol]):
                set_following.add("'$'")
            for key in self.__grammar:
    ...
    def __get_set_of_the_following(self, key, rule, symbol):
        ...
        if leftover is not None:
            print('set_foll key:{}:rule:{}:symbol:{}'.format(key, rule, symbol))
            epsilon = False
            aux_set = set()
            if len(leftover) > 0:
            ...
            if len(leftover) == 0 or epsilon:
                ...
                if key in self.__dict_of_the_following:
                    aux_set = self.__dict_of_the_following[key]
                    set_following |= aux_set
    ...
    def __get_prediction_set(self, key, rule):
        ...
        prediction_set = self.__get_set_of_the_first(key, rule)
